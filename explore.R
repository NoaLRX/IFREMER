...
svm_model_opt <- train(ts_HKE1567_adj ~ ., data = train, method = "svmRadial", preProcess = c("center", "scale"), tuneGrid = svm_grid)
print(svm_model_opt) # On obtient sigma = 0.01 et C = 10.

svm_model_best <- svm(ts_HKE1567_adj ~ ., data = train, kernel = "radial", gamma = 0.1, cost = 0.35, scale = TRUE)
